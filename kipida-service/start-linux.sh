#!/usr/bin/env bash
# KiPIDA service launcher for Linux.
...
# Prefer python3, fall back to python (must be 3.x).
PYTHON=""
...
if [ -z "$PYTHON" ]; then
  ...
  echo "[KiPIDA] Debian/Ubuntu: sudo apt install python3 python3-pip"
  echo "[KiPIDA] Fedora:        sudo dnf install python3 python3-pip"
  echo "[KiPIDA] Arch:          sudo pacman -S python python-pip"
  read -rp "Press Enter to exit..."
  exit 1
fi
...
if ! "$PYTHON" -m pip --version >/dev/null 2>&1; then
  ...
  echo "[KiPIDA] Debian/Ubuntu: sudo apt install python3-pip"
  echo "[KiPIDA] Or try:        $PYTHON -m ensurepip --upgrade"
  read -rp "Press Enter to exit..."
  exit 1
fi
